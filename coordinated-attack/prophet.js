...
export var Prophet = function() {
    ...
    if (this.me.turn > 3 && offenseFlag == 2) { //to avoid conflicting with castle locations 
        this.castleTalk(2); //2 signifies "i am a ranger on defense mode"
    }
    ...
    if (this.fuel > 500 && about_to_attack == 0 && offenseFlag == 2) {
        about_to_attack = 1;
    }
    if (about_to_attack == 1 && countdown > 0 && offenseFlag == 2) {
        countdown--;
    }
    if (countdown == 0 && offenseFlag == 2) {
        offenseFlag = 1;
        this.log("ATTACK MODE ACTIVATE!!!");
        target = null;
    }
