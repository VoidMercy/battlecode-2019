        ...
        for (var i = 0; i < nearbyrobots.length; i++) {
            if (nearbyrobots[i].team == this.me.team) {
                if (nearbyrobots[i].unit == SPECS.PREACHER) {
                    distance = this.distance([this.me.x, this.me.y], [nearbyrobots[i].x, nearbyrobots[i].y]);
                    if (distance > biggest) {
                        biggest = distance;
                    }
                }
            } else {
                if (nearbyrobots[i].unit == SPECS.CASTLE || nearbyrobots[i].unit == SPECS.CHURCH) {
                    distance = this.distance([this.me.x, this.me.y], [nearbyrobots[i].x, nearbyrobots[i].y]);
                    if (distance < closestdistance) {
                        closestcastle = nearbyrobots[i];
